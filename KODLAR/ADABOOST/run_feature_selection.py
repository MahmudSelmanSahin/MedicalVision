"""
AdaBoost — Özellik Seçimi (ORİJİNAL 353-sütun veri)  [TAM OPTİMİZASYON]
Giriş : MODELLER/ADABOOST/ABLASYON/ADABOOST_best_by_panel.csv
        + VERİLER/ABLASYON_MODEL_BAZLI/{scenario}/{PANEL}_*.csv
Çıkış : MODELLER/ADABOOST/ORİJİNAL_VERİ/{YÖNTEM}/SONUCLAR|GRAFIKLER

_temiz versiyonundan tek farkı: veriyi tek TEMİZLENMİŞ dosyadan değil,
...
Optimizasyon katmanları (sonuçta ayrı kolonlar halinde gösterilir):
  1. mcc_default_05  : n_estimators=50, lr=1.0, threshold=0.5 (baz)
  2. mcc_tuned_05    : Optuna(30 trial) ile param, threshold=0.5
  3. mcc (ana metrik): Optuna params + OOF-tabanlı MCC-optimal threshold

AdaBoost ağaç tabanlıdır → ölçeklemeye gerek yoktur; EMBEDDED yöntem
AdaBoost'un kendi feature_importances_ değerini kullanır.
...
try:
    ...
    OPTUNA_OK = False
    print("UYARI: optuna yüklü değil → varsayılan parametreler kullanılacak.  pip install optuna")

MODEL_NAME   = "ADABOOST"
DATA_ROOT    = ROOT / "VERİLER" / "ABLASYON_MODEL_BAZLI"
OUT_BASE     = MODELS_ROOT / MODEL_NAME / "ORİJİNAL_VERİ"
ABLASYON_SRC = MODELS_ROOT / MODEL_NAME / "ABLASYON"   # run_ablation.py best_by_panel buraya yazar
ABLASYON     = OUT_BASE / "ABLASYON"
N_FEATURES = 30
N_CV       = 5      # OOF fold sayısı
N_TRIALS   = 30     # Optuna deneme sayısı

METHOD_PATHS: dict[str, tuple[str, str]] = {
    "FILTER_ANOVA":                                ("FILTER",                      "ANOVA"),
    "FILTER_MUTUAL_INFO":                          ("FILTER",                      "MUTUAL_INFO"),
    "WRAPPER_RFE":                                 ("WRAPPER",                     "RFE"),
    "EMBEDDED":                                    ("EMBEDDED",                    "ADABOOST_IMPORTANCE"),
    "FILTER_ANOVA + WRAPPER_RFE":                  ("FILTER + WRAPPER",            "ANOVA + RFE"),
    "FILTER_ANOVA + EMBEDDED":                     ("FILTER + EMBEDDED",           "ANOVA + ADABOOST_IMPORTANCE"),
    "FILTER_MUTUAL_INFO + WRAPPER_RFE":            ("FILTER + WRAPPER",            "MUTUAL_INFO + RFE"),
    "FILTER_MUTUAL_INFO + EMBEDDED":               ("FILTER + EMBEDDED",           "MUTUAL_INFO + ADABOOST_IMPORTANCE"),
    "WRAPPER_RFE + EMBEDDED":                      ("WRAPPER + EMBEDDED",          "RFE + ADABOOST_IMPORTANCE"),
    "FILTER_ANOVA + WRAPPER_RFE + EMBEDDED":       ("FILTER + WRAPPER + EMBEDDED", "ANOVA + RFE + ADABOOST_IMPORTANCE"),
    "FILTER_MUTUAL_INFO + WRAPPER_RFE + EMBEDDED": ("FILTER + WRAPPER + EMBEDDED", "MUTUAL_INFO + RFE + ADABOOST_IMPORTANCE"),
}
...
def _tune_ada(X_tr: np.ndarray, y_tr: np.ndarray) -> dict:
    if not OPTUNA_OK:
...
def _oof_evaluate(X: np.ndarray, y: np.ndarray, params: dict, n_splits: int = N_CV):
    ...
    oof_prob_default = np.zeros(len(y))   # n_estimators=50, lr=1.0
    oof_prob_tuned   = np.zeros(len(y))   # Optuna params

    for tr_idx, te_idx in skf.split(X, y):
        m_def = _build_ada(_DEFAULT_PARAMS)
        m_def.fit(X[tr_idx], y[tr_idx])
        oof_prob_default[te_idx] = m_def.predict_proba(X[te_idx])[:, 1]

        m_opt = _build_ada(params)
        m_opt.fit(X[tr_idx], y[tr_idx])
        oof_prob_tuned[te_idx] = m_opt.predict_proba(X[te_idx])[:, 1]
...
# ---------------------------------------------------------------------------
# Veri yükleme — best_by_panel'den seçilen senaryo
# ---------------------------------------------------------------------------

def load_best_scenarios() -> dict[str, tuple[str, Path]]:
    best_path = ABLASYON_SRC / f"{MODEL_NAME}_best_by_panel.csv"
    ...
    for _, row in best.iterrows():
        ...
        if csvs:
            ...
            print(f"  UYARI: {DATA_ROOT / scenario}/{panel}_*.csv bulunamadı.")
    return result
...
def compute_embedded(X, y, names):
    model = _build_ada(dict(n_estimators=200, learning_rate=0.5, max_depth=2))
    model.fit(X, y)
...
def main() -> None:
    ...
    for panel, (scenario, csv_path) in best_scenarios.items():
        ...
        best_params = _tune_ada(X[:n_tr][:, em_idx], y.values[:n_tr])
        print(f" → n_est={best_params.get('n_estimators')}, lr={best_params.get('learning_rate'):.4f}, depth={best_params.get('max_depth')}")

        panel_cache[panel] = {"X": X, "names": names, "y": y, "scenario": scenario,
    ...
    for method in METHODS:
        ...
        for panel, cache in panel_cache.items():
            try:
                ...
                m_final = _build_ada(cache["best_params"])
                ...
                row = {
                    ...
                    "best_n_estimators": cache["best_params"].get("n_estimators"),
                    "best_learning_rate": cache["best_params"].get("learning_rate"),
                    "best_max_depth": cache["best_params"].get("max_depth"),
