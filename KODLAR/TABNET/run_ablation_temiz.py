"""
TabNet — Adım 1: Ön-İşleme Ablasyonu (TEMİZLENMİŞ veri)
Giriş : VERİLER/ABLASYON_TEMİZLENMİŞ/TABNET_*/{PANEL}_*.csv
Çıkış : MODELLER/TabNet/TEMİZLENMİŞ/ABLASYON/

CatBoost/run_ablation.py kalıbının TabNet + TEMİZLENMİŞ veri uyarlamasıdır.
Farklar:
  - Veri kaynağı ABLASYON_TEMİZLENMİŞ
  - Senaryo öneki büyük harf "TABNET_"
  - TabNet bir sinir ağıdır: sayısal, ÖLÇEKLENMİŞ ve NaN'sız girdi ister.
    TabNet senaryoları zaten ordinal + scaler (Robust/MinMax/Standard) + imputer ile
    üretildiği için hazırdır; güvenlik için kalan NaN 0 ile doldurulur.
  - Dengesizlik için fit(weights=1) → dengeli örnekleme (auto_class_weights karşılığı)

Gereksinim: pip install pytorch-tabnet torch
Not: TabNet eğitimi yavaştır; 9 senaryo × 4 panel = 36 fit.
...
def fit_predict(x_train, y_train, x_test):
    ...
    model.fit(
        ...
        patience=0,            # eval_set yok → erken durdurma kapalı, sabit epoch
        batch_size=bs,
        virtual_batch_size=vbs,
        weights=1,             # dengeli sınıf örneklemesi
    )
