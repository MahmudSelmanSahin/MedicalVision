"""
LightGBM — k Analizi (ORİJİNAL 353-sütun veri)  [TAM OPTİMİZASYON]
Giriş : MODELLER/LightGBM/ABLASYON/LightGBM_best_by_panel.csv
        + VERİLER/ABLASYON_MODEL_BAZLI/{scenario}/{PANEL}_*.csv
Çıkış : MODELLER/LightGBM/ORİJİNAL_VERİ/ABLASYON/k_analysis_summary.csv
    ...
    def objective(trial):
        ...
        for tr_idx, te_idx in skf.split(X_tr, y_tr):
            ...
            try:
                ...
                X_fit, X_val, y_fit, y_val = X_f, X_f[:1], y_f, y_f[:1]
...
def compute_scores(X, y, names):
    ...
    lgbm.fit(X, y)
    em_scores = lgbm.feature_importances_.astype(float)
...
def fit_lgbm_compare(X_full, sel_idx, y, best_params):
    ...
    return (y_te, y_pred, y_prob, len(y_tr_np), len(y_te_np),
            best_thr, mcc_default_05, mcc_t05, mcc_topt, n_trees)
...
def main() -> None:
    ...
    for panel, (scenario, csv_path) in best_scenarios.items():
        ...
        print(f" leaves={best_params.get('num_leaves','?')} lr={best_params.get('learning_rate','?'):.4f}")
            ...
            for method in ALL_METHODS:
                ...
                try:
                    (y_te, y_pred, y_prob, n_tr, n_te,
                     best_thr, mcc_def, mcc_t05, mcc_topt, n_trees) = fit_lgbm_compare(
                    ...
                    rows.append({
                        ...
                        "best_threshold": best_thr, "n_trees_used": n_trees,
