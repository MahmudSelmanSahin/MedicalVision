"""
TabPFN — Adım 2: Özellik Seçimi (Feature Selection)

TabPFN kendi feature_importance üretmez → EMBEDDED için RF surrogate kullanılır.
TabPFN RFE için çok yavaş → WRAPPER_RFE için RF proxy kullanılır.
...
Yöntemler (atomik):
  ...
  EMBEDDED            : RandomForest surrogate feature_importances_
...
def to_numeric_array(df: pd.DataFrame, cols: list[str]) -> tuple[np.ndarray, list[str]]:
    """TabPFN verisi zaten numeric; NaN kaldıysa 0 ile doldur."""
...
def compute_embedded(X: np.ndarray, y: np.ndarray, names: list[str]) -> list[str]:
    """EMBEDDED: TabPFN feature importance yok → RF surrogate."""
