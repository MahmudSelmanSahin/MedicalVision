"""
KNN — Adım 1: Ön-İşleme Ablasyonu (TEMİZLENMİŞ veri)
Giriş : VERİLER/ABLASYON_TEMİZLENMİŞ/KNN_*/{PANEL}_*.csv
Çıkış : MODELLER/KNN/TEMİZLENMİŞ/ABLASYON/

XGBOOST/run_ablation_temiz.py kalıbının KNN uyarlamasıdır.
Farklar:
  - Model: KNeighborsClassifier
  - Senaryo öneki "KNN_" (build_ablation_temiz.py çıktısıyla eşleşir)
  - Senaryo CSV'leri zaten onehot kodlu + ölçeklenmiş → KNN doğrudan kullanır.
  - n_neighbors eğitim örnek sayısına göre kısıtlanır.

Çalıştırma (KNN klasöründen):
...
def prepare_xy(csv_path):
    ...
    # Senaryo CSV'leri kodlu/ölçekli; güvenlik için kalan object sütunları sayıya çevir.
...
def fit_predict(x_train, y_train, x_test):
    k = min(11, max(1, len(x_train) - 1))
    model = KNeighborsClassifier(n_neighbors=k, weights="distance", p=2, n_jobs=-1)
