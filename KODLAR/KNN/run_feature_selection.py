"""
KNN — Özellik Seçimi (ORİJİNAL 353-sütun veri)  [TAM OPTİMİZASYON]
Giriş : MODELLER/KNN/ABLASYON/KNN_best_by_panel.csv
        + VERİLER/ABLASYON_MODEL_BAZLI/{scenario}/{PANEL}_*.csv
Çıkış : MODELLER/KNN/ORİJİNAL_VERİ/{YÖNTEM}/SONUCLAR|GRAFIKLER
...
Optimizasyon katmanları (sonuçta ayrı kolonlar halinde gösterilir):
  1. mcc_default_05  : k=5, uniform, threshold=0.5 (baz)
  2. mcc_tuned_05    : Optuna(30 trial) ile k+weights+p, threshold=0.5
  3. mcc (ana metrik): Optuna params + OOF-tabanlı MCC-optimal threshold

KNN mesafe tabanlıdır → StandardScaler ile ölçeklenir. KNN'in doğal gömülü
önemi olmadığından EMBEDDED yöntem RandomForest vekil önemini kullanır.
...
def _tune_knn(X_tr: np.ndarray, y_tr: np.ndarray) -> dict:
    """Optuna ile n_neighbors, weights ve p araması (eğitim verisi 3-fold CV)."""
...
def _oof_evaluate(X: np.ndarray, y: np.ndarray, params: dict, n_splits: int = N_CV):
    ...
    oof_prob_default = np.zeros(len(y))   # k=5, uniform
    ...
    for tr_idx, te_idx in skf.split(X, y):
        ...
        # Varsayılan model
        m_def = _build_knn(dict(n_neighbors=5, weights="uniform", p=2), len(tr_idx))
        m_def.fit(X_tr_sc, y[tr_idx])
        oof_prob_default[te_idx] = m_def.predict_proba(X_te_sc)[:, 1]

        # Optimize model
        m_opt = _build_knn(params, len(tr_idx))
...
def main() -> None:
    ...
    for panel, (scenario, csv_path) in best_scenarios.items():
        ...
        best_params = _tune_knn(X_em_sc[:n_tr], y.values[:n_tr])
    ...
    for method in METHODS:
        ...
        for panel, cache in panel_cache.items():
            try:
                ...
                m_final = _build_knn(cache["best_params"], len(X_tr))
                ...
                save_confusion_matrix_png(
                    ...
                    f"{MODEL_NAME} | {method} | {panel}\nk={cache['best_params'].get('n_neighbors')} thr={oof['oof_best_threshold']:.2f}")
