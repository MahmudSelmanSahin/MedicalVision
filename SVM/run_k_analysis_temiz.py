"""
SVM — k Analizi (TEMİZLENMİŞ veri)  [TAM OPTİMİZASYON]
Giriş : VERİLER/TEMİZLENMİŞ/YARISMA_TRAIN_{PANEL}_temiz.csv
Çıkış : MODELLER/SVM/TEMİZLENMİŞ/ABLASYON/k_analysis_summary.csv
...
def _tune_panel(X_tr: np.ndarray, y_tr: np.ndarray) -> dict:
    """Panel başına bir kez Optuna/GridSearchCV ile C ve gamma bul."""
    n = len(X_tr)
    if n > 600:
...
def compute_scores(X, y, names):
    ...
    lsvc = LinearSVC(C=1.0, max_iter=3000, class_weight="balanced",
                     random_state=RANDOM_STATE, dual=False)
    lsvc.fit(X_sc, y)
    em_scores = np.abs(lsvc.coef_).mean(axis=0)
...
def fit_svm_compare(X_full, sel_idx, y, best_params):
    """Hem varsayılan hem optimize SVM eğit → karşılaştırmak için."""
    ...
    # Varsayılan model (C=1.0, threshold=0.5)
    ...
    m_opt = SVC(kernel="rbf", class_weight="balanced", probability=True,
                cache_size=200, random_state=RANDOM_STATE, **best_params)
...
def main() -> None:
    ...
    for panel, df in panel_dfs.items():
        ...
        print(f"  C={best_params['C']:.4f}  gamma={best_params.get('gamma','?')}")
            ...
            for method in ALL_METHODS:
                ...
                try:
                    (y_te, y_pred, y_prob, n_tr, n_te,
                     best_thr, mcc_def05, mcc_t05, mcc_topt) = fit_svm_compare(
